...
if (isset($_GET["id"])) {
	...
	$db = new UzivatelDB();
	$vysledek = $db->smazUzivatele($_GET["id"]);
	if ($vysledek == true) {
		echo "<h2 class= 'spravne'>Příspěvek byl smazán</h2>";
		header("Location: Uzivatel-administrace.php");
	} else {
		echo "<h2 class= 'chyba'>Příspěvek nebyl smazán</h2>
                <a href='menu.php'>Zpět na hlavní stánku</a>";
	}
}
?>
<!DOCTYPE html>
...
<body>
	...
	<main>
		...
		<div class="kartyUzivatelu">
			...
			$uzivatel->vypisUzivateleAdministraceBezSmazani();
			?>
		</div>
		<h1>Administrace zaměstnanců:</h1>
		<div class="kartyUzivatelu">
			<?php

			$db = new UzivatelDB();
			$uzivatele = $db->nactiZamestnance();
			foreach ($uzivatele as $uzivatel) {
				$uzivatel->vypisUzivateleAdministrace();
			}
			?>
