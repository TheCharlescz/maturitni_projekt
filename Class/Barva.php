...
public function vypisFunkceBarvy($produkt_id) {
    if (isset($_POST[$this->barva])) {
        ...
        $db = new BarvaDB();
        $id = $db->vlozBarvuDoProduktu($produkt_id, $this->id);
        //if ($id === '0') {
...
public function vypisFunkciUpravyBarvy($produkt_id, $pocet = 1) {
        spl_autoload_register(function ($trida) {
            include_once "Class/$trida.php";
        });
			...
			if (!isset($_POST["$this->id"])) {
				$db = new BarvaDB();
				$db->odeberBarvuProduktu($produkt_id, $this->id);
				$pocet++;
			}
			if (isset($_POST["$this->id"])) {
				$db = new BarvaDB();
				$db->vlozBarvuDoProduktu($produkt_id, $this->id);
			$pocet++;
			}
			return $pocet;
}
